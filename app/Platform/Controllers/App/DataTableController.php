...
class DataTableController extends \App\Http\Controllers\Controller
{
    ...
    */
    ...
    public function getDataList(Request $request)
    {
        ...
        foreach ($tableFilters as $filter) {
            ...
            if ($filter['type'] == 'relation') {
                ...
                if ($relation['type'] == 'hasOne' || $relation['type'] == 'belongsTo') {
                    if ($permission == 'all') {
                        $items = $model::with($relation['with'])->getRelation($relation['with'])->selectRaw($relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function ($query) use ($where) {
                            if ($where != '') return $query->whereRaw($where);
                        })->orderBy($relation['orderBy'], $relation['order'])->get();
                    } else {
                        $items = $model::with($relation['with'])->getRelation($relation['with'])->selectRaw('' . $relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function ($query) use ($where) {
                            if ($where != '') return $query->whereRaw($where);
                        })->where('created_by', auth()->user()->id)->orderBy($relation['orderBy'], $relation['order'])->get();
                    }
                }

                if ($relation['type'] == 'belongsToMany') {
                    if ($permission == 'all') {
                        $items = DB::table($relation['table'])->selectRaw($relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function ($query) use ($where) {
                            if ($where != '') return $query->whereRaw($where);
                        })->orderBy($relation['orderBy'], $relation['order'])->get();
                    } else {
                        $items = DB::table($relation['table'])->selectRaw('' . $relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function ($query) use ($where) {
        ...
        //   return array_merge($selectRaw, $extraQueryColumns);
        // Query model
        // (new Model)->newQuery();
      ...
      */
        ...
        foreach ($records as $key => $record) {
            if ($record['role'] == 'Customer') {
                ...
                // return $number = (array_key_exists('whatsapp', $record)) ? $record['whatsapp'] : '';
                ...
                $records[$key]['customer_number'] = $formatted_number;
                $records[$key]['business_name'] = $formatted_number;
            }
        }
    ...
    public function getExport(Request $request)
    {
        ...
        */
            ...
            switch ($ext) {
                case 'xlsx';
                    $blob = \Maatwebsite\Excel\Facades\Excel::raw(new $exportClass, \Maatwebsite\Excel\Excel::XLSX);
                    break;
                case 'xls';
                    $blob = \Maatwebsite\Excel\Facades\Excel::raw(new $exportClass, \Maatwebsite\Excel\Excel::XLS);
                    break;
                case 'csv';
                    $blob = \Maatwebsite\Excel\Facades\Excel::raw(new $exportClass, \Maatwebsite\Excel\Excel::CSV);
                    break;
                case 'html';
                    $blob = \Maatwebsite\Excel\Facades\Excel::raw(new $exportClass, \Maatwebsite\Excel\Excel::HTML);
                    break;
            }
