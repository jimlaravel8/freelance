                  ...
                  <v-col cols="12" sm="9" md="6">
                    <v-card flat tile class="content-tabs-card pa-5 mb-0 mb-md-12">
                      <v-card-text v-if="$auth.user().role === 4 && !$auth.user().premium">
                        <p
                          class="fg--text subtitle-1 font-weight-medium"
                        >{{ $t('account_inactive') }}</p>
                      </v-card-text>
                      <v-card-text v-else>
                        <v-alert :value="verifyCodeForm.success" type="success" class="mb-5">
                          <i18n path="redeem_points_success">
                            ...
                            <template #transactions>
                              ...
                              >{{ $t('transactions').toLowerCase() }}</router-link>
                            </template>
                          </i18n>
                        </v-alert>
                            ...
                            <v-row no-gutters>
                              ...
                              <v-col cols="4">
                                <v-btn
                                  ...
                                  height="56px"
                                  :loading="verifyCodeForm.loading"
                                  :disabled="verifyCodeForm.loading || invalid"
                                >
                                  {{ $t('verify') }}
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            depressed
            :loading="verifyCodeForm.loading"
            :disabled="verifyCodeForm.loading"
            @click="verifyCodeDialog = false;processCode()"
      ...
      this.axios
        ...
        .then(response => {
          if (response.data.status === "success") {
            ...
            this.msg.value = this.formatCurrency(
              response.data.value,
      ...
      this.axios
        ...
        .then(response => {
          if (response.data.status === "success") {
            ...
            this.msg.value = this.formatCurrency(
              response.data.redeem.value,
              response.data.redeem.currency
            );
