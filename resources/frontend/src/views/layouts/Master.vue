      ...
      <v-container class="inner-container">
        <v-row no-gutters>
          ...
          <div class="d-flex align-center">
            ...
            <router-link
              v-if="$init.app.logo == '' || $init.app.logo == null"
              :to="{name: 'home'}"
              style="max-width: 156px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
              class="subtitle-1 font-weight-medium white--text text-decoration-none"
              v-html="$init.app.name"
            ></router-link>
          </div>
            ...
            <v-card tile max-width="460">
              <v-list dense tile class="pt-0">
                <v-list-item class="px-3 py-2">
                  ...
                  <v-list-item-content>
                    <v-list-item-title
                      v-html="$auth.user().first_name + ' ' + $auth.user().last_name"
                    />
                ...
                <v-list-item-group>
                  ...
                  <v-list-item
                    v-if="$auth.impersonating() && parseInt($auth.user().role) != 4"
                    @click="$auth.unimpersonate({redirect: {name: 'admin.users'}})"
                  >
                    <v-list-item-icon>
                      <v-icon>mdi-power</v-icon>
                    </v-list-item-icon>
                  ...
                  <v-list-item
                    v-if="$auth.impersonating() && parseInt($auth.user().role) == 4"
                    @click="$auth.unimpersonate({redirect: {name: 'business.staff'}})"
                  >
                    <v-list-item-icon>
                      <v-icon>mdi-power</v-icon>
                    </v-list-item-icon>
          ...
          <!-- computer -->
          <div
            class="d-none d-sm-flex d-md-flex d-lg-flex d-xl-flex align-center ml-2"
            v-if="!$auth.check()"
          >
            <v-btn
              color="NavFg NavBg--text"
          ...
          <!-- phone -->
          <div
            class="d-flex d-sm-none d-md-none d-lg-none d-xl-none align-center ml-2"
            v-if="!$auth.check()"
          >
            <v-btn
              color="NavFg NavBg--text"
              ...
              tile
              class="no-caps"
              :to="{name: pathname == 'register' ? 'login' : 'register'}"
            >
              <v-icon
                class="d-flex"
              >{{(this.pathname == 'register' || this.pathname == 'register-business') ? 'mdi-account' : 'mdi-pencil-outline'}}</v-icon>
            </v-btn>
          </div>
        </v-row>
      </v-container>
          ...
          <v-col cols="6" xs="6" lg="3">
            ...
            <ul class="nav-list">
              <li>
                <router-link
                  :to="{name: 'home'}"
                  class="NavFg--text text-decoration-none"
                >{{ $t('home') }}</router-link>
              </li>
              <li>
                <router-link
