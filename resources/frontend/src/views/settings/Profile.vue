    ...
    <v-container class="inner-container">
        <v-card class="mt-md-12 mt-0 content-tabs-wrapper" tile elevation="0">
            <v-tabs :color="($store.state.app.dark) ? 'white' : 'black'" class="px-5 content-tabs" icons-and-text height="100" slider-size="4" show-arrows>
                ...
                <v-tab class="no-caps" :to="{ name: 'settings.business' }" :ripple="false" v-if="$auth.user().role === 3">
                    {{ $t('business') }}
                    <v-icon size="24">mdi-briefcase</v-icon>
                </v-tab>
                <v-tab class="no-caps" :to="{ name: 'settings.business.subscription' }" :ripple="false" v-if="$auth.user().role === 3">
                    Billing
                    ...
                    <v-form :model="form1" id="form1" lazy-validation @submit.prevent="submitForm('form1')" autocomplete="off" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
                        <v-card-text>
                            ...
                            <v-tabs-items v-model="tab" :touchless="false">
                                    ...
                                    <v-container class="pa-0">
                                        <v-row no-gutters>
                                            <v-col cols="12" sm="9" md="6">
                                                ...
                                                <x-text-field v-model="form1.whatsapp" ref="form1.whatsapp" id="form1.whatsapp" :label="$auth.user().role === 2 ? 'Phone Number' : 'Contact Number'" rules="required|min:7|max:7" dense :prefix="form1.country_code" />
                                ...
                                <v-tab-item :transition="false" :reverse-transition="false" :value="'localization'">
                                    <v-container class="pa-0">
                                        <v-row no-gutters>
                                            <v-col cols="12" sm="9" md="6">
                                                <x-autocomplete v-model="form1.locale" :items="locales" item-value="0" item-text="1" label="Language" :name="$t('locale')" rules="required" :hint="$t('locale_hint')" :persistent-hint="true" dense />
                                ...
                                <v-tab-item :transition="false" :reverse-transition="false" :value="'password'">
                                    <v-container class="pa-0">
                                        <v-row no-gutters>
                                            <v-col cols="12" sm="9" md="6">
                                                <x-password v-model="form1.new_password" ref="form1.new_password" id="form1.new_password" :label="$t('change_password')" :name="$t('password')" rules="min:8|max:24" :hint="$t('leave_empty_for_no_change')" :persistent-hint="true" dense />
                                            </v-col>
                                        </v-row>
                                    </v-container>
                                </v-tab-item>
                            ...
                            <v-container class="pa-0">
                                <v-row no-gutters>
                                    <v-col cols="12" md="8">
                                        <x-password v-model="form1.current_password" ref="form1.current_password" id="form1.current_password" :label="$t('current_password')" :name="$t('current_password')" rules="required|min:8|max:24" :hint="$t('current_password_info')" persistent-hint dense />
                                    </v-col>
                                </v-row>
                            </v-container>
                        </v-card-text>
