...
<div class="viewContainer">
    <v-container class="inner-container">
        <v-card class="mt-md-12 mt-0 content-tabs-wrapper" tile elevation="0">
            ...
            <v-divider />
            <v-tabs v-model="tab" :color="($store.state.app.dark) ? 'white' : 'black'" slider-size="2" class="mt-5 px-5 content-tabs" show-arrows active-class="font-weight-bold">
            ...
            <v-card flat tile class="content-tabs-card pa-5 mb-0 mb-md-12">
                <ValidationObserver ref="form1" v-slot="{ invalid }">
                    <v-form :model="form1" id="form1" lazy-validation @submit.prevent="submitForm('form1')" autocomplete="off" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
                        <v-card-text>
                            <v-alert :value="form1.hasError && !form1.success" type="error" class="mb-8">{{ form1.error }}</v-alert>
                            <v-alert :value="form1.success" type="success" class="mb-4">{{ $t('update_success') }}</v-alert>
                            <v-tabs-items v-model="tab" :touchless="false">
                                      ...
                                      <v-overlay v-if="hover" absolute>
                                        <v-btn-toggle rounded>
                                          <v-btn
                                            ...
                                            rounded
                                          >
                                            <v-icon class="mr-1" size="14">mdi-upload</v-icon>
                                            {{ $t('upload') }}
                                          </v-btn>
                                          <v-btn
                                            v-if="showDeleteCover"
                                            @click="form1.cover_media_url = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAA9JREFUeNpiuHbtGkCAAQAFCAKDZcGh3gAAAABJRU5ErkJggg=='; form1.cover_media_changed = true; showDeleteCover = false"
                                            x-small
                                            color="red"
                                            rounded
                                          >
                              ...
                              />-->
                                                ...
                                                <x-text-field v-model="form1.name" ref="form1.name" id="form1.name" :label="$t('name')" :name="$t('name')" rules="required|min:1|max:96" dense />
                                                <x-text-field v-model="form1.short_description" ref="form1.short_description" id="form1.short_description" :label="$t('short_description')" :name="$t('description')" rules="nullable|max:100" dense />
                                                <x-autocomplete v-model="form1.cid" ref="form1.cid" id="form1.cid" :items="categories" item-value="cid" item-text="name" :label="'Business Category'" :name="'Business Category'" rules="required" dense :hint="'Select a category within the list to update category.'" persistent-hint :loading="loading"></x-autocomplete>
                                            </v-col>
                                        </v-row>
                                    </v-container>
                                </v-tab-item>
                                <v-tab-item :transition="false" :reverse-transition="false" value="localization">
                                    <v-container class="pa-0">
                                        <v-row no-gutters>
                                            <v-col cols="12" sm="9" md="6">
                                                <x-autocomplete v-model="form1.currency" ref="form1.currency" id="form1.currency" :items="currencies" item-value="0" item-text="1" :label="$t('currency')" :name="$t('currency')" rules="required" dense :hint="$t('business_currency_hint')" persistent-hint />
                                            </v-col>
                                        </v-row>
                                    </v-container>
                                </v-tab-item>
                        ...
                        <v-card-actions class="mx-2">
                            <v-btn color="ctaBg ctaFg--text" tile depressed x-large :loading="form1.loading" :disabled="form1.loading || invalid" type="submit" class="no-caps mb-2" style="min-width: 200px">
                                {{ $t('update') }}
...
export default {
            ...
            .then(response => {
                ...
                this.axios
                    ...
                    .then(response => {
                        ...
                        this.showDeleteCover =
                            this.form1.cover_media_url == null ? false : true;
                        this.form1.currency = business.currency;
                        this.form1.cid = business.cid;
    ...
    methods: {
        async getCategories() {
            const body = await this.axios.get("/category/all");
