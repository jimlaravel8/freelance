                                    ...
                                    <ValidationObserver ref="form1" v-slot="{ invalid }">
                                        <v-form :model="form1" @submit.prevent="submit('form1')" autocomplete="off" method="post">
                                            <v-card flat tile class="content-tabs-card pa-5 mb-0 mb-md-12">
                                                <v-card-text>
                                                    <h1 class="fg--text display-2 font-weight-medium mb-4" style="line-height: 4rem; max-width: 300px">{{ $t('registration_user_title') }}</h1>
                                                    <p class="body-1">{{ $t('registration_user_head') }}</p>
...
export default {
    ...
    created() {
        ...
        // Tab images
        this.tabImgs = [
            this.$init.config.schemeAndHost + "/img/sign-up-customer-tab.jpg"
        ];
                    ...
                    this.$auth.login({
                        ...
                        success() {
                            ...
                            if (parseInt(this.$auth.user().role) == 2)
                                customer_type = "customer.earn";
                            if (parseInt(this.$auth.user().role) == 3)
                                customer_type = "business.issue-points";
