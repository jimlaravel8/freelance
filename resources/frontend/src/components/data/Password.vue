...
<v-card>
    <v-toolbar :tabs="tabCount > 1" flat v-if="!loadingForm">
        ...
        <template v-slot:extension v-if="tabCount > 1 && ! limitReached">
            <v-tabs slot="extension" v-model="selectedTab" show-arrows v-if="tabCount > 1">
    ...
    <ValidationObserver ref="formModel" v-slot="{ invalid }">
        <v-form :model="formModel" v-if="!loadingForm && ! limitReached" @submit.prevent="submitForm('formModel')" autocomplete="off" method="post" id="formModel" accept-charset="UTF-8" enctype="multipart/form-data">
            <v-divider></v-divider>
                ...
                <v-tabs-items v-model="selectedTab" :touchless="true" class="mx-2">
                    <v-tab-item :value="tab_index" v-for="(tab, tab_index) in form.items" :key="tab_index" :eager="true">
                        ...
                        <v-card v-if="Object.keys(form.items[tab_index].subs).length > 1" class="mb-3 elevation-1">
                            <v-tabs v-if="Object.keys(form.items[tab_index].subs).length > 1" v-model="selectedSubTab[tab_index]" hide-slider color="primary" background-color="grey lighten-4" show-arrows>
                  ...
                  -->
                                <div v-for="(form_item, form_index) in sub.items" :key="'form_' + form_index">
                                    <div v-if="form_item.type == 'description'">
                                        <v-sheet v-html="form_item.text" class="pa-3 mb-3 subtitle-1 elevation-1" color="grey lighten-4"></v-sheet>
                                    </div>

                                    <div v-if="form_item.type == 'text' || form_item.type == 'email' || form_item.type == 'number'">
                                        <x-text-field :type="form_item.type" v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :prepend-inner-icon="form_item.icon" :placeholder="form_item.placeholder" :hint="form_item.hint" :persistent-hint="true" :prefix="form_item.prefix" :suffix="form_item.suffix" />
                                    </div>

                                    <div v-if="form_item.type == 'password'">
                                        <x-password v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :prepend-inner-icon="form_item.icon" :placeholder="form_item.placeholder" :hint="form_item.hint" :persistent-hint="true" :prefix="form_item.prefix" :suffix="form_item.suffix" />
                                    </div>

                                    <div v-if="form_item.type == 'boolean'">
                                        <x-checkbox v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :hint="form_item.hint" :persistent-hint="true" />
                                    </div>

                                    <div v-if="form_item.type == 'wysiwyg'">
                                        <x-editor v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :rules="form_item.validate" :config="form_item.config" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :hint="form_item.hint" :persistent-hint="true" />
                                    </div>

                                    <div v-if="form_item.type == 'enum'">
                                        <x-autocomplete v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :hint="form_item.hint" :persistent-hint="true" :placeholder="form_item.placeholder" :prepend-inner-icon="form_item.icon" :items="relations[form_item.column].items" :loading="formModel[form_item.column + '_loading']" :prefix="form_item.prefix" :suffix="form_item.suffix" item-text="val" item-value="pk" hide-no-data hide-selected clearable />
                                    </div>

                                    <div v-if="form_item.type == 'date'">
                                        <x-date-picker v-model="formModel[form_item.column]" :ref="form_item.column" :id="form_item.column" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :prepend-inner-icon="form_item.icon" :placeholder="form_item.placeholder" :hint="form_item.hint" :locale="$auth.user().locale.substring(0,2)" _format="form_item.format" :persistent-hint="true" :prefix="form_item.prefix" :suffix="form_item.suffix" />
                                    </div>

                                    <div v-if="form_item.type == 'relation' && form_item.relation.type == 'belongsToMany'">
                                        <x-autocomplete v-model="formModel[form_item.relation.with]" :ref="form_item.relation.with" :id="form_item.relation.with" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :name="form_item.text" :hint="form_item.hint" :persistent-hint="true" :placeholder="form_item.placeholder" :prepend-inner-icon="form_item.icon" :items="relations[form_item.relation.with].items" :prefix="form_item.prefix" :suffix="form_item.suffix" item-text="val" item-value="pk" hide-no-data hide-selected chips multiple small-chips deletable-chips />
                                    </div>
                                    ...
                                    <div v-if="form_item.type == 'image' || form_item.type == 'avatar'">
                                                    ...
                                                    <v-fade-transition>
                                                        <v-overlay v-if="hover" absolute>
                                                            <v-btn-toggle rounded>
                                                                <v-btn @click="pickFile(form_item.column)" x-small color="primary" rounded>
                                                                    <v-icon size="14">mdi-folder-open</v-icon>
                                                                </v-btn>
                                                                <v-btn @click="formModel[form_item.column + '_media_name'] = ''; formModel[form_item.column + '_media_url'] = ''; formModel[form_item.column + '_media_changed'] = true;" x-small color="red" rounded>
                                        ...
                                        <x-text-field @click="pickFile(form_item.column)" type="text" readonly v-model="formModel[form_item.column + '_media_name']" :ref="form_item.column + '_placeholder'" :id="form_item.column + '_placeholder'" :rules="form_item.validate" :label="(form_item.required) ? form_item.text + $t('_required_') : form_item.text" :prepend-inner-icon="form_item.icon" :placeholder="form_item.placeholder" :hint="form_item.hint" :persistent-hint="true" :prefix="form_item.prefix" :suffix="form_item.suffix">
    ...
    methods: {
        ...
        saveForm() {
            ...
            for (let field in this.formModel) {
                if (
                    this.$_.endsWith(field, "_media_url") ||
                    this.$_.endsWith(field, "_media_name") ||
                    this.$_.endsWith(field, "_media_file") ||
                    field == "avatar"
                ) {
                ...
                .catch(err => {
                    if (typeof err.response !== "undefined") {
                        ...
                        let el =
                            ...
                            null;
                        let tab =
                            el !== null ? el[0].$parent.$parent.$parent.$vnode.key : null;
                        if (tab !== null) this.selectedTab = tab;
