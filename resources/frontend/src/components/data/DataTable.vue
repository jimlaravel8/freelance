  ...
  <div class="fill-height">
    ...
    <v-card flat v-show="loaded" color="transparent">
      <v-toolbar
        flat
        color="transparent"
        class="toolbar-content-no-padding mt-2 mb-4"
        v-if="filteredResults || rows.length > 0"
        height="40"
      >
      ...
      <div
        v-if="!filteredResults && !loading && loaded && rows.length == 0"
      ...
      <v-toolbar
        flat
        color="transparent"
        v-show="filters.length > 0 && loaded && ((rows.length == 0 && filteredResults) || parseInt(rows.length) > 0)"
      >
        <v-spacer></v-spacer>
      ...
      >
        <template :slot="'item.' + item.value" slot-scope="props" v-for="(item, index) in headers">
          <div
            v-if="index != Object.keys(headers).length - 1 || ! settings.actions"
            :style="item.style"
            :key="'col_' + index"
          >
            <div v-if="item.type === 'boolean'" class="text-center">
              <div
                v-if="props.item[item.value] === 1 || props.item[item.value] === '1' || props.item[item.value] === true"
              >
            ...
            <div v-else-if="item.type === 'link'">
              <a
                ...
                target="_blank"
              >{{ props.item[item.value].replace('http://', '').replace('https://', '').replace('//', '') }}</a>
            </div>
            ...
            <div v-else-if="item.type === 'date_time' || item.type === 'date'">
              <span
                v-html="parseDateTime(props.item[item.value], item.format, item.color_is_past, item.color_is_future)"
              ></span>
            </div>
              ...
              <v-tooltip top v-if="item.action">
                <template v-slot:activator="{ on }">
                  <v-btn
                    ...
                    icon
                    small
                    @click="executeAction(item.action, props.item.uuid)"
                  >
            ...
            <v-card outlined>
              ...
              <div v-if="billings.loaded && billings.status_code == 200">
                <v-input
                  ...
                  color="black"
                  :messages="['Due Payment Date (Net 7)']"
                ...
                <v-input
                  ...
                  prepend-icon="mdi-cash"
                  color="black"
                  :messages="[`Points Issuance Sessions: ${this.billings.transaction_details.earnings.count}`]"
                >
                ...
                <v-input
                  class="ma-5"
                  color="black"
                  :messages="[`Points Redemption Sessions: ${this.billings.transaction_details.spendings.count}`]"
                >
            ...
            this.axios
              ...
              .then(res => {
                if (res.data.status === "success") {
                  this.reloadData();
                  this.$root.$snackbar(
                    this.$t("items_deleted", { items: res.data.deleted })
                  );
                }
              })
              .catch(err => {
