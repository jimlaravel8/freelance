                  ...
                  <v-col cols="12" md="12">
                    <v-card flat tile class="content-tabs-card pa-5 mb-0 mb-md-12">
                      <ValidationObserver ref="form1" v-slot="{ invalid }">
                        ...
                        >
                          <v-card-text>
                            <v-alert :value="!$auth.user().premium" type="info" tile class="mb-5">
                            ...
                            <v-row>
                              <v-col cols="12" sm="6" class="py-0 order-2 order-sm-1">
                                ...
                                <x-text-field
                                  ...
                                  dense
                                />
                                <h3 class="title mb-3">Points Issue Limits</h3>
                                <x-text-field
                                  :disabled="!$auth.user().premium"
                                  type="number"
                                ...
                                <x-text-field
                                  ...
                                  dense
                                />
                                <h3 class="title mb-3">Points Redemption Limits</h3>
                                <x-text-field
                                  :disabled="!$auth.user().premium"
                                  type="number"
                                  ...
                                  <div class="mb-5 fg--text body-2">
                                    ...
                                    <p v-if="getRedeemableTestPoints() < 0">
                                      <i18n path="test_scenario_2">
                                        <template
                                          #points
                                        >{{ formatNumber(Math.abs(getRedeemableTestPoints())) }}</template>
                                        <template
                                          #pointsValue
                                        >{{ formatCurrency(Math.abs(getRedeemableTestPoints()) * form1.point_value) }}</template>
                                        <template #months>{{ form1.points_expiration_months }}</template>
                                        <template
                                          #redemptionPoints
                                        >{{ formatNumber(form1.min_points_per_redemption) }}</template>
                                        <template
                                          #redemptionPointsValue
                                        >{{ formatCurrency(form1.min_points_per_redemption * form1.point_value) }}</template>
                                      </i18n>
                                    </p>
                                    <p v-if="getRedeemableTestPoints() > 0">
                          ...
                          <v-card-actions class="mx-2">
                            <v-btn
                              ...
                              :disabled="form1.loading || invalid || !$auth.user().premium"
                              type="submit"
                              class="no-caps mb-2"
...
export default {
  ...
  created() {
    this.axios
      ...
      .then(response => {
        ...
        this.form1.min_points_per_redemption =
          business.min_points_per_redemption;
        this.form1.max_points_per_redemption =
  ...
  methods: {
    ...
    getTestPoints() {
      ...
      if (parseInt(points) < parseInt(this.form1.min_points_per_purchase))
        points = this.form1.min_points_per_purchase;
      if (parseInt(points) > parseInt(this.form1.max_points_per_purchase))
        points = this.form1.max_points_per_purchase;
      return points;
    },
    getRedeemableTestPoints() {
      let points = this.getTestPoints();
      if (parseInt(points) < parseInt(this.form1.min_points_per_redemption))
        points -= this.form1.min_points_per_redemption;
      if (parseInt(points) > parseInt(this.form1.max_points_per_redemption))
        points = this.form1.max_points_per_redemption;
      return points;
    },
