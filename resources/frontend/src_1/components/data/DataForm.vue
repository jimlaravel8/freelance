  ...
  <v-card>
    ...
    <div v-if="!loadingForm && limitReached">
      <v-card-text class="pt-0">
        <p class="subtitle-1">
          <span
            v-if="max != 0"
          >You have reached the maximum of {{ max }} {{ translations.items_lowercase }}.</span>
          <i18n v-if="max == 0" path="premium_account_required">
            <template #premium_account>
              <router-link
                :to="{name: 'settings.business.subscription'}"
              >{{ $t('premium_account').toLowerCase() }}</router-link>
            </template>
          </i18n>
        </p>
      </v-card-text>
    ...
    <ValidationObserver ref="formModel" v-slot="{ invalid }">
      ...
      >
        ...
        <v-card-text
          :style="{ 'height': settings.dialog_height + 'px' || 'auto', 'max-width': '800px', 'overflow-y': 'auto' }"
        >
              ...
              <v-tabs-items v-model="selectedSubTab[tab_index]" :touchless="true">
                ...
                <div v-for="(sub, sub_index) in tab.subs">
                  ...
                  <div v-for="(form_item, form_index) in sub.items" :key="'form_' + form_index">
                    ...
                    <div
                      v-if="form_item.type == 'text' || form_item.type == 'email' || form_item.type == 'number'"
                    >
                      <x-text-field
                        :type="form_item.type"
                    ...
                    <div
                      v-if="form_item.type == 'relation' && form_item.relation.type == 'belongsToMany'"
                    >
                      <x-autocomplete
                    ...
                    <div
                      v-if="form_item.type == 'relation' && (form_item.relation.type == 'hasOne' || form_item.relation.type == 'belongsTo')"
                    >
                      <x-autocomplete
                        v-model="formModel[form_item.column]"
                        :ref="form_item.column"
                    ...
                    <div v-if="form_item.type == 'image' || form_item.type == 'avatar'">
                      <v-hover>
                        <template v-slot:default="{ hover }">
                          <v-img
                            :src="formModel[form_item.column + '_media_url']"
                            class="mt-1 mb-3 elevation-3"
                            :class="form_item.class"
                            contain
                            :style="{'width': form_item.image.thumb_width, 'height': form_item.image.thumb_height, 'max-width': form_item.image.thumb_max_width, 'max-height': form_item.image.thumb_max_height}"
                            v-if="formModel[form_item.column + '_media_url']"
                          >
                            <v-fade-transition>
                              <v-overlay v-if="hover" absolute>
                                <v-btn-toggle rounded>
                                  ...
                                  <v-btn
                                    @click="formModel[form_item.column + '_media_name'] = ''; formModel[form_item.column + '_media_url'] = ''; formModel[form_item.column + '_media_changed'] = true;"
                                    x-small
                                    color="red"
                                    rounded
                                  >
                      ...
                      >
                        <template slot="append">
                          <v-icon
                            v-if="formModel[form_item.column + '_media_name'] != ''"
                            @click="formModel[form_item.column + '_media_name'] = ''; formModel[form_item.column + '_media_url'] = ''; formModel[form_item.column + '_media_changed'] = true;"
                          >mdi-close</v-icon>
                        </template>
                      </x-text-field>
        ...
        <v-card-actions v-if="!loadingForm">
          ...
          <v-btn
            ...
            large
            @click="$emit('data-list-events', {'closeDialog': true, 'reload': false})"
            class="no-caps"
          >{{ $t('close') }}</v-btn>
          <v-btn
            ...
            :disabled="form.loading || invalid"
            large
            type="submit"
            class="no-caps"
          >{{ (uuid === null) ? $t('create') : $t('save') }}</v-btn>
        </v-card-actions>
      </v-form>
    </ValidationObserver>
